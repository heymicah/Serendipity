...
const EditEvent = ({ isOpen, onClose, eventId, onEventUpdated }) => {
  const { token } = useAuth();
  ...
  const fetchEventData = async () => {
    try {
      ...
      // Parse school years and genders from comma-separated strings
      const schoolYears = event.school_years ? event.school_years.split(', ').map(s => s.trim()) : [];
      const genders = event.genders ? event.genders.split(', ').map(g => g.trim()) : [];
    ...
    <div className="create-event-overlay" onClick={onClose}>
      <div className="create-event-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {loading ? (
          ...
          <div className="error-message" style={{ textAlign: 'center', padding: '2rem' }}>{errors.fetch}</div>
        ) : (
